...
/**
 * Normalize /v1/responses bodies: OpenAI uses output_text or nested output[].
 * Many gateways return HTTP 200 with chat-like or partial shapes instead.
 */
export function extractTextFromResponsesBody(data) {
  ...
  const loose =
    ...
    null;
  if (loose && String(loose).trim()) return String(loose).trim();
...
export async function callOpenAIResponses(
  ...
  async function runChatCompletionsFallback() {
    try {
  ...
  if (!r.ok) {
    ...
    return await runChatCompletionsFallback();
  }

  const text = extractTextFromResponsesBody(data);
  if (text) return text;

  // HTTP 200 but gateway omitted output_text (common on some proxies vs local Node).
  return await runChatCompletionsFallback();
}
